...
class ReportAction(models.Model):
    ...
    def _render_jpk_xlsx(self, report_ref, docids, data):
        ...
        return (
            report_model.with_context(active_model=report_sudo.model).sudo(False).create_xlsx_report(docids, data)  # noqa
        )
    ...
    def _render_jpk_xml(self, report_ref, docids, data):
        ...
        report_model = self.env[report_model_name]
        return (
            report_model.with_context(active_model=report_sudo.model).sudo(False).create_xml_report(docids, data)  # noqa
        )
