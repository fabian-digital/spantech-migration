...
# Extend the AML_IMPORT_BLACKLIST to block import of fields
# that are added by optional modules (e.g.account_followup)
AML_IMPORT_BLACKLIST = [
    "amount_residual",
...
class AmlImport(models.TransientModel):
    _name = "aml.import"
                    ...
                    if fmt == "char":
                        ...
                        try:
                            ...
                            if err_msg:
                                err_msg += "\n"
                            err_msg += _(
                                "Incorrect value '%(cell)s' for field '%(hf)s' !"
                            ) % {
                                "cell": cell.value,
                                "hf": hf,
                            }
    ...
    def aml_import(self):
        ...
        wiz_dict = {}
        self._init_wiz_dict(wiz_dict, move)
        ...
        vals = self._process_vals(wiz_dict, move, vals, imported_lines)
        ...
        if wiz_dict["err_log"]:
            ...
            self._update_move(wiz_dict, move, vals, imported_lines)
    ...
    def _init_wiz_dict(self, wiz_dict, move):
    ...
    def _input_fields(self, wiz_dict):
        """
        ...
        res = {
            ...
            "balance": {"method": self._handle_balance, "field_type": "float"},
            ...
            "analytic_distribution": {
                "method": self._handle_analytic_distribution,
                "field_type": "json",
            },
    ...
    def _get_orm_fields(self, wiz_dict):
        ...
        orm_fields = aml_mod._fields
        blacklist = (
            ["move_id"]
            + models.MAGIC_COLUMNS
            + [aml_mod.CONCURRENCY_CHECK_FIELD]
            + AML_IMPORT_BLACKLIST
        )
        wiz_dict["orm_fields"] = {
            f: orm_fields[f]
            for f in orm_fields
            if f not in blacklist and orm_fields[f].store and not orm_fields[f].related
        }

    def _process_header(self, wiz_dict, header_fields):
        wiz_dict["field_methods"] = self._input_fields(wiz_dict)
            ...
            if hf in wiz_dict["field_methods"] and wiz_dict["field_methods"][hf].get(
                ...
                if not wiz_dict["field_methods"][hf].get("field_type"):
                    raise UserError(
                        _(
                            f"Programming Error:\nMissing formatting "
                            f"info for column '{hf}'."
                        )
                    )
                continue

            if hf not in wiz_dict["orm_fields"] and hf not in [
                wiz_dict["orm_fields"][f].string.lower() for f in wiz_dict["orm_fields"]
            ]:
                _logger.error(
                    _(
                        f"{self._name}, unsupported field '{hf}' "
                        f"found while importing move lines"
                    )
                )
    ...
    def _handle_orm_integer(
        ...
        if not aml_vals.get(orm_field):
            ...
            if val:
                ...
                if val is False:
                    msg = _(
                        f"Incorrect value '{line[field]}' for field '{field}' of "
                        f"type Integer !"
                    )
    ...
    def _handle_orm_float(self, wiz_dict, field, line, move, aml_vals, orm_field=False):
        ...
        if not aml_vals.get(orm_field):
            ...
            if val:
                ...
                if val is False:
                    msg = _(
                        f"Incorrect value '{line[field]}' for field '{field}' "
                        f"of type Numeric !"
                    )
                    self._log_line_error(wiz_dict, line, msg)
                else:
    ...
    def _handle_credit(self, wiz_dict, field, line, move, aml_vals):
        if "credit" not in aml_vals:
            ...
            if isinstance(credit, str):
                ...
                if isinstance(credit, str):
                    msg = _("Incorrect Credit value !")
                    self._log_line_error(wiz_dict, line, msg)
            aml_vals["credit"] = credit

    def _handle_balance(self, wiz_dict, field, line, move, aml_vals):
        if not aml_vals.get("balance"):
            balance = line[field]
            if isinstance(balance, str):
                balance = str2float(balance, self.decimal_separator)
                if isinstance(balance, str):
                    msg = _("Incorrect Balance value !")
                    self._log_line_error(wiz_dict, line, msg)
            aml_vals["balance"] = balance
    ...
    def _handle_analytic_distribution(self, wiz_dict, field, line, move, aml_vals):
        if not aml_vals.get("analytic_distribution"):
            ...
            output_val = {}
            for k in input_val:
                if k not in ("id", "code", "name"):
                    msg = _("Incorrect value '%(val)s' for field '%(field)s' !") % {
                        "val": input_val,
                        "field": field,
                    }
                    self._log_line_error(wiz_dict, line, msg)
                    return
                analytic_accounts = ana_mod.search(domain + [(k, "=", input_val[k][0])])
                if len(analytic_accounts) == 1:
                    output_val[analytic_accounts.id] = input_val[k][1]
                elif not analytic_accounts:
                    msg = _("No Analytic Account found for '%(fld)s : %(val)s' !") % {
                        "fld": k,
                        "val": input_val[k][0],
                    }
                    self._log_line_error(wiz_dict, line, msg)
                    ...
                    msg = _(
                        "Multiple Analytic Accounts found that match "
                        "with '%(fld)s : %(val)s' !"
                    ) % {
                        "fld": k,
                        "val": input_val[k][0],
                    ...
                    self._log_line_error(wiz_dict, line, msg)
                    return
            aml_vals["analytic_distribution"] = output_val
    ...
    def _process_line_vals(self, wiz_dict, line, move, aml_vals):
        ...
        if aml_vals.get("balance") and isinstance(aml_vals["balance"], float):
            balance = aml_vals["balance"]
            del aml_vals["balance"]
            debit = balance > 0 and balance or 0
            credit = balance < 0 and -balance or 0
            if (aml_vals["debit"] and aml_vals["debit"] != debit) or (
                aml_vals["credit"] and aml_vals["credit"] != credit
            ):
                msg = _("Debit/credit values different from balance !")
                self._log_line_error(wiz_dict, line, msg)
            else:
                aml_vals["debit"] = debit
                aml_vals["credit"] = credit
        ...
        if (
            isinstance(aml_vals["debit"], float)
            and isinstance(aml_vals["credit"], float)
            and (
                aml_vals["debit"] < 0
        ...
        if (
            not aml_vals.get("account_id")
            and aml_vals.get("product_id")
            and wiz_dict["doc_type"] in ("sale", "purchase")
        ):
            product = (
                self.env["product.product"]
                .with_company(move.company_id)
                .browse(aml_vals["product_id"])
            )
            accounts = product.product_tmpl_id.get_product_accounts(
                fiscal_pos=move.fiscal_position_id
            )
            if wiz_dict["doc_type"] == "purchase":
                aml_vals["account_id"] = accounts["expense"].id
            else:
                aml_vals["account_id"] = accounts["income"].id
    ...
    def _process_line_vals_currency(self, wiz_dict, line, move, aml_vals):
        if "currency_id" in aml_vals:
            ...
            debit = (
                isinstance(aml_vals.get("debit"), float) and aml_vals["debit"] or 0.0
            )
            credit = (
                isinstance(aml_vals.get("credit"), float) and aml_vals["credit"] or 0.0
            )
    ...
    def _process_vals(self, wiz_dict, move, vals, imported_lines):
        """
        Use this method if you want to check/modify the
        input values dict before calling the move write() method
        """
        sum_debit = sum(x[2]["debit"] for x in vals if isinstance(x[2]["debit"], float))
        sum_credit = sum(
            x[2]["credit"] for x in vals if isinstance(x[2]["credit"], float)
        )
    ...
    def _update_move(self, wiz_dict, move, vals, imported_lines):
