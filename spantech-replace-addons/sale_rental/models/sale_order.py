...
class SaleOrderLine(models.Model):
    ...
    def _get_number_of_months(self):
        ...
        if not self.end_date or not self.start_date:
            return qty
        fake_end_date = self.end_date + relativedelta(day=31)
        for dt in rrule(
                MONTHLY, dtstart=self.start_date, until=fake_end_date
        ):
            days_of_month = monthrange(dt.year, dt.month)
            if self.start_date.month == self.end_date.month:
                qty = self.rental_qty * (
                        self.number_of_days / days_of_month[1]
                )
                ...
                qty += self.rental_qty * (
                        self.end_date.day / days_of_month[1]
                )
            else:
                qty += 1
        return qty
    ...
    def _get_number_of_years(self):
        ...
        if self.number_of_days < days_of_year:
            ...
            for dt in rrule(
                    YEARLY, dtstart=self.start_date, until=fake_end_date
            ):
                ...
                if self.start_date.year == dt.year:
                    ...
                    qty += (
                            self.rental_qty
                            * (
                                    self.end_date - dt.date() + timedelta(days=1)
                            ).days
                            / days_of_year
                    )
                else:
