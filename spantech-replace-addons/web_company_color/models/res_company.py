...
class ResCompany(models.Model):
    ...
    @api.model
    def _scss_get_sanitized_values(self, values):
        values.update(
            {
    ...
    @api.model
    def _scss_generate_content(self, values):
        # ir.attachment need files with content to work
        if not values:
            return "// No Web Company Color SCSS Content\n"
        return self.SCSS_TEMPLATE % self._scss_get_sanitized_values(values)

    def scss_get_url(self, multi_company=False):
        self.ensure_one()
        if multi_company:
            return URL_SCSS_GEN_TEMPLATE % 0
        else:
    ...
    def scss_create_or_update_attachment(self):
        ...
        for record in self:
            ...
            values = dict(record.company_colors or {})
            datas = base64.b64encode(
                record._scss_generate_content(values).encode("utf-8")
            )
